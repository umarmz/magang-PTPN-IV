...
<?php
$tahun=['txttahun'];
$sql_tahun = mysqli_query($konek, "Select DISTINCT tb_peserta.tahun_lulus FROM tb_tracer, tb_peserta WHERE tb_tracer.nisn=tb_peserta.nisn ;") or die
(mysqli_error($con));
                  ...
                  <div class="modal fade" id="cetak" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                            <h4 class="modal-title" id="myModalLabel">Asal Sekolah</h4>
                          </div>
                          <div class="modal-body">
                            <!--FORM MODAL-->
                            <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                  ...
                                  <div class="col-lg-10">
                                  <select class="form-control" name="txtidsekolah" id="txtidsekolah">
                              <option value="">- Pilih -</option>
                              <?php
                              $sekolah=['txttahun'];
                              $sql_sek = mysqli_query($konek, "select distinct id_sekolah, nama_sekolah from tb_sekolah") or die
                              (mysqli_error($con));
                              while($data_sek = mysqli_fetch_array($sql_sek)) {
                              echo '<option value="'.$data_sek['nama_sekolah'].'">'.$data_sek['nama_sekolah'].' </option>';              
                              }?>
                              </select>
                                  </div>
                                </div>
                                <div class="form-group">
                                  <div class="col-lg-10 col-lg-offset-2">
                                    <button type="reset" class="btn btn-default" data-dismiss="modal">Batal</button>
                                    <button type="submit" id="cetak" name="cetak" class="btn btn-primary">Pilih</button>
                                  </div>
                                </div>
                                </fieldset>
                            </form>
...
$sql_tampil = mysqli_query($konek,"SELECT tb_tracer.id_alumni, tb_peserta.nisn, tb_peserta.nama, tb_sekolah.id_sekolah, tb_sekolah.nama_sekolah, status, nama_instansi, tb_peserta.tahun_lulus FROM tb_tracer, tb_peserta, tb_sekolah WHERE tb_tracer.nisn=tb_peserta.nisn AND tb_peserta.id_sekolah=tb_sekolah.id_sekolah AND tb_tracer.status='Bekerja' AND tb_sekolah.id_sekolah LIKE'".$query."' ORDER BY id_alumni ASC");				
}else{
$sql_tampil =mysqli_query($konek,"SELECT DISTINCT tb_tracer.id_alumni, tb_peserta.nisn, tb_peserta.nama, tb_sekolah.nama_sekolah, status, nama_instansi, tb_peserta.tahun_lulus FROM tb_tracer, tb_peserta, tb_sekolah WHERE tb_tracer.nisn=tb_peserta.nisn AND tb_peserta.id_sekolah=tb_sekolah.id_sekolah AND tb_tracer.status='Bekerja' ORDER BY id_alumni DESC");		
}
